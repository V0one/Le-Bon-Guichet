...
export async function lireAnnuaire(url: string, signal: AbortSignal) {
  ...
  while (offset < 10000) {
    ...
    if (resultats.length >= page.total_count)
      return { results: resultats, total_count: page.total_count };
    if (
      page.results.length === 0 ||
      prochaine.searchParams.get("limit") === "1"
    ) {
...
export function chercherOrganismesParCommune(
  codeInsee: string,
  signal: AbortSignal,
) {
