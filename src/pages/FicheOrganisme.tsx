...
export function FicheOrganisme() {
  ...
  const resultats = etat.statut === 'succes' ? lirePageAnnuaire(etat.donnees).results : [];
  const organisme = resultats.length > 0 ? normaliserFiche(resultats[0]) : null;
  const titre = !url || (etat.statut === 'succes' && !organisme)
    ? 'Page introuvable' : organisme?.nom || 'Fiche organisme';
  useEffect(() => {
    document.title = `${titre} - Le Bon Guichet`;
  }, [titre]);

  if (!url || (etat.statut === 'succes' && !organisme)) return <NotFound />;
  if (etat.statut === 'erreur') return <EtatErreur message={etat.message} onReessayer={relancer} />;
  if (!organisme) return etat.statut === 'chargement' ? <Chargement /> : <p>Préparation de la fiche…</p>;
  return <>
    <h1>{organisme.nom}</h1>
    <h2>Coordonnées</h2>
    <h3>Adresse</h3><p>{organisme.adresse}</p>
    <h3>Téléphone</h3>
