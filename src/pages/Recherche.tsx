...
import { filtrerOrganismesParType, lirePageAnnuaire } from '../domain/recherche';
...
export function Recherche() {
  ...
  return <>
    ...
    <p>Trouvez les administrations implantées dans une commune et leurs coordonnées.</p>
    <form onSubmit={event => { event.preventDefault(); (lieu ? annuaire : geo).relancer(); }}>
      <Select label="Type d’organisme" nativeSelectProps={{ value: type, onChange: event => setType(event.target.value) }}>
        ...
        : <><h2>{organismes.length} organisme{organismes.length > 1 ? 's' : ''} trouvé{organismes.length > 1 ? 's' : ''}</h2>
          <ul className="fr-grid-row fr-grid-row--gutters">{organismes.map(organisme => <li className="fr-col-12 fr-col-md-6" key={organisme.id}>
            <Card title={organisme.nom} titleAs="h3" desc={organisme.adresse}
              linkProps={{ to: `/organismes/${encodeURIComponent(organisme.id)}` }} enlargeLink />
          </li>)}</ul></>)}
    </section>
  </>;
}
